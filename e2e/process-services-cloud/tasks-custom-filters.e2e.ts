...
describe('Task filters cloud', () => {
    ...
    describe('Filters', () => {
        ...
        it('[C290045] Should display only tasks with Assigned state when Assigned is selected from state dropdown', () => {
            ...
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsDisplayed(assignedTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(createdTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(completedTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(deletedTaskName);
        });
        ...
        it('[C290061] Should display only tasks with Completed state when Completed is selected from state dropdown', () => {
            ...
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsDisplayed(completedTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(assignedTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(createdTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(deletedTaskName);
        });
        ...
        it('[C290139] Should display only tasks with all states when All is selected from state dropdown', () => {
            ...
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsDisplayed(deletedTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsDisplayed(assignedTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsDisplayed(createdTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsDisplayed(completedTaskName);
        });
        ...
        it('[C290154] Should display only tasks with suspended states when Suspended is selected from state dropdown', () => {
            ...
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsDisplayed(suspendedTasks.list.entries[0].entry.id);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(deletedTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(createdTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(completedTaskName);
            tasksCloudDemoPage.taskListCloudComponent().checkContentIsNotDisplayed(assignedTaskName);
